    ...
    extends InputVisitor
{
    ...
    private final PropertyPath createPath( final Input input )
    {
        ...
        return PropertyPath.from( parentElement, getElementsAsArray( input.getPath() ) );
    }

    private String[] getElementsAsArray( final FormItemPath formItemPath )
    {
        return StreamSupport.stream( formItemPath.spliterator(), false ).toArray( String[]::new );
    }
}
